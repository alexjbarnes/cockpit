...
describe("processEvents", () => {
  ...
  describe("thinking", () => {
    it("creates a thinking block", () => {
      const state = makeState({ currentAssistantMsgId: "msg-1" });
      const events: ParsedEvent[] = [makeEvent({ type: "thinking", text: "hmm", tokens: 5 })];
      processEvents(events, state, defaults);

      expect(state.pendingBlocks).toHaveLength(1);
      expect(state.pendingBlocks[0]).toEqual({ type: "thinking", text: "hmm", tokens: 5, redacted: undefined });
    });

    it("appends to existing thinking block", () => {
      const state = makeState({ currentAssistantMsgId: "msg-1" });
      state.pendingBlocks.push({ type: "thinking", text: "first", tokens: 3 });
      const events: ParsedEvent[] = [makeEvent({ type: "thinking", text: " second", tokens: 4 })];
      processEvents(events, state, defaults);
