---
description: Implement a refined cockpit issue. Branches off next in an isolated git worktree, writes the code following the approved plan, runs build/lint/tests, runs the completeness-reviewer agent first to confirm every acceptance criterion has implementing code, self-reviews with the code-reviewer agent (up to 4 rounds), runs the ui-reviewer agent for screenshots when UI changed, opens a PR, waits for CI to pass, starts a live test server and posts its URL for the human to verify, then moves the issue to Code Review. Use when asked to implement, build, or code up a cockpit issue, e.g. "implement CK-12".
---
...
### 1. Read the issue, resolve the repo, and name the branch
- `mcp__cockpit-config__get_issue` for the key. One call returns the title, full description (the plan), labels, status, and every comment. The Code Review gate often leaves approval conditions or change requests in comments; they override the plan where they conflict.
- Resolve the repo: `list_projects`, match the issue's `projectId`, and use that project's `repoPath` if set. Otherwise use the session's current working directory (today's behaviour, and the only case while cockpit is the sole project). Call the resolved absolute path `<REPO>` — every git/gh/npm command below runs against it, never against wherever the shell cwd happens to be.
- Derive the branch name from the key: `issue/<ISSUE-ID>` (e.g. `issue/CK-12`). There is no tracker-provided branch name to read (Linear generated one; the native issue record does not); this fixed convention replaces it, and `accept-issue` derives the identical name the same way when it needs to find or recreate the branch.
...
- **First implementation**: no PR exists for the branch yet (`gh pr list --head issue/<ISSUE-ID>` is empty). Do the full flow below.
- **Rework**: a PR already exists for the branch AND the latest human comment (posted after the last "Review server" comment, at the Code Review gate) asks for changes. That comment is the contract for this pass.

**In rework the prior attempt is ~right and the human flagged a delta — fix only that delta. Do NOT re-walk the plan or rebuild the branch off `next`.** It changes three later steps: step 4 restores the prior branch instead of resetting to `next`; step 5 makes only what the comment asks; steps 8-10 review just the delta. Verify, CI watch, the test server, and the Code Review transition are unchanged. Re-implementing the whole plan and re-running every review loop on a small feedback delta is the single biggest waste of the human's time after a reject.
...
Set the issue status to `Implementation` via `update_issue` (key + status).

### 4. Create an isolated worktree off next
Work in a git worktree so the implementation is isolated from `<REPO>`.

Clear any stale worktree first (the reaper usually removed it on rejection, but be safe), then create it according to the mode from step 2a. Name the worktree `<NAME>-<ISSUE-ID>`, sibling to `<REPO>`, where `<NAME>` is `<REPO>`'s basename (e.g. `cockpit`, matching today's naming exactly since `<REPO>` falls back to this checkout):

```
git -C <REPO> worktree remove --force ../<NAME>-<ISSUE-ID> 2>/dev/null || true
...
```
git -C <REPO> worktree add ../<NAME>-<ISSUE-ID> -B issue/<ISSUE-ID> origin/next
```
...
```
git -C <REPO> fetch origin issue/<ISSUE-ID>
git -C <REPO> worktree add ../<NAME>-<ISSUE-ID> -B issue/<ISSUE-ID> origin/issue/<ISSUE-ID>
git -C ../<NAME>-<ISSUE-ID> rebase origin/next
```

Resolve any rebase conflicts (usually trivial, far cheaper than rebuilding); if one is genuinely unresolvable, fall back to a full re-implement off `next`. Record the rebased HEAD (`git -C ../<NAME>-<ISSUE-ID> rev-parse HEAD`) as the **rework base** — the code review later diffs only against it. Do all subsequent work in that worktree by **absolute path** or `git -C <worktree>`, never by relying on a `cd`. The job's cwd is cockpit's own checkout (this repo, which also serves the live instance and is used interactively) — not necessarily `<REPO>` once a second project is in play — and the shell cwd does not persist between Bash calls, so a `cd <worktree> && …` in one call does not carry to the next, and a later bare `git add` / edit / build then lands wherever the cwd reset to and pollutes it. The branch is based on `next`, and the PR will target `next`.
...
2. **Hard cap: 4 rounds.** If criteria are still genuinely unmet after the fourth, stop. Proceed and carry the unmet criteria to the Code Review transition as a comment so the human knows exactly what is missing.
...
**Post the full review of every round as a comment** on the issue via `add_issue_comment`, before fixing anything. Post the agent's complete output verbatim, all four severity buckets with file:line, breaking example, and suggested fix for each finding, plus the verdict, not a summary. Head each comment with the round number (e.g. "Code review round 2/4"). Do this for every round, including a PASS. This is the observability trail for tuning the implement skill.
...
2. **Hard cap: 4 rounds.** If Critical/High findings still remain after the fourth review, stop iterating. Proceed but carry the unresolved findings to the Code Review transition.
...
**When to stop iterating** (first match wins):
1. **PASS or only Medium/Low remain.** Apply Medium fixes. Proceed.
2. **Hard cap: 4 rounds.** If Critical/High UI findings still remain after the fourth review, stop. Proceed and carry the unresolved findings to the Code Review transition; the screenshots are already attached for the human to judge.
...
- **Still red after 3 attempts, or CI never completes:** stop. Do NOT start the test server (step 12) or move to Code Review (step 13). Leave the issue in `Implementation` and post a comment naming the failing check(s) with the run URL. A red PR must never reach the Code Review gate.
...
### 13. Link the PR and transition to Code Review
Do this last, after the test server is up and its URL is posted, so the human never lands on Code Review before the link exists.
- Attach the PR to the issue: `add_issue_attachment` with `key`, `url: <pr-url>`, `title: <pr-title>`.
- Set the issue status to `Code Review` via `update_issue` (key + status). `Code Review` is its own named status, distinct from the refinement-side `Plan Review` gate, so setting it by name is unambiguous — no group-qualified state ID to resolve first. This is a requirements/feature gate: a human checks the change does what the issue asked and behaves correctly. The code, UI, and completeness reviews already ran inline; this is not a repeat of them.
...
### 14. Do not clean up
Leave the worktree and the test server running. They are the human's review surface, and the `accept-issue` skill reaps them (kills the server, removes the worktree) when the issue is accepted, or when it leaves Code Review without being accepted (e.g. rejected back to Implementation Ready). The pushed branch and PR are the durable artifacts; the worktree and server are ephemeral review aids.

## Rules
- The plan is the contract. Follow it. Record deviations in the PR body; do not improvise silently.
- Comments at the Code Review gate override the plan where they conflict.
...
- Work in the worktree off `next`, addressing it by absolute path or `git -C <worktree>`. Never write to, stage in, build in, or save artifacts into the job's cwd (this checkout, shared with the live instance) — the cwd resets to it between Bash calls, which is how stray staged changes and files leak into it. The PR targets `next`.
...
- CI is the gate to the human, not the local run. After opening the PR, watch its GitHub checks (`gh pr checks --watch`) and only proceed to the test server and Code Review when they pass. If CI cannot be made green in 3 fix attempts, leave the issue in Implementation with a comment naming the failing checks. Never move a red PR to Code Review.
- Four review rounds max. Still failing after four: open the PR, route to Code Review, and comment the unresolved findings with reasons.
- If the change touches UI, run the ui-reviewer agent (up to 4 rounds, same as code review) so screenshots of the change are attached to the issue before the human gate. Still failing after four: carry the findings to Code Review with the screenshots attached.
- Run the completeness-reviewer agent first (before code and UI review, up to 4 rounds) as a coverage check that every acceptance criterion has implementing code or a test; purely-visual criteria are deferred to the ui-reviewer and the human. Genuinely unmet after four: carry the gaps to Code Review as a comment.
