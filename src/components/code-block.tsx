...
export function CodeBlock({ code, language, dark, fullHeight, wrap }: CodeBlockProps) {
  ...
  const overflowX = wrap ? "overflow-x-hidden" : "overflow-x-auto";
  const wrapHighlighted = wrap ? " code-block-wrap [&_pre]:whitespace-pre-wrap [&_pre]:[overflow-wrap:anywhere]" : "";
  const wrapPlain = wrap ? " whitespace-pre-wrap [overflow-wrap:anywhere]" : "";
  const renderedHtml = wrap && html ? html.replace(/\n/g, "") : html;

  if (html) {
    return (
      <div
        className={`code-block-lines ${overflowX} rounded text-[11px] leading-relaxed overflow-y-auto [&_pre]:!bg-transparent [&_pre]:p-2 [&_pre]:m-0${wrapHighlighted}${fullHeight ? "" : " max-h-64"}`}
      >
  ...
  return (
    <pre
      className={`${overflowX} rounded bg-muted/50 p-2 text-[11px] leading-relaxed overflow-y-auto${wrapPlain}${fullHeight ? "" : " max-h-64"}`}
    >
      {strippedCode}
    </pre>
  );
