...
function resolveAgentPath(name: string, scope: string, cwd?: string | null): string | null {
  if (!NAME_RE.test(name)) return null;
  if (scope === "user") {
    return path.join(homedir(), ".claude", "agents", `${name}.md`);
  }
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ name: string }> }) {
  ...
  const filePath = resolveAgentPath(name, scope, cwd);
  if (!filePath) {
    return NextResponse.json({ error: "Invalid agent name or scope" }, { status: 400 });
  }
