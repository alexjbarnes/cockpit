...
// Native installs (mise, brew, binary) typically track this ahead of npm's latest dist-tag.
const GCS_LATEST_URL = "https://storage.googleapis.com/claude-code-dist-86c565f3-f756-42ad-8dfa-d59b1c096819/claude-code-releases/latest";
...
export async function GET() {
  ...
  try {
    const res = await fetch(GCS_LATEST_URL, { signal: AbortSignal.timeout(5000) });
    if (res.ok) {
      const text = (await res.text()).trim();
      if (text) latest = text;
    }
  } catch {
    // GCS unreachable, fall through to npm fallback
  }
