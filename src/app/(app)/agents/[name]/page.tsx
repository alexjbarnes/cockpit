...
export default function AgentEditorPage({
  ...
  usePageHeader(isNew ? "New Agent" : "Edit Agent", true);
  ...
  useEffect(() => {
    ...
    fetch(`/api/agents/${encodeURIComponent(rawName)}?${params}`)
      .then((res) => {
      ...
      .catch(() => {
        setFeedback("Failed to load agent");
      })
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      if (isNew) {
        const params = new URLSearchParams({ scope });
        if (cwd) params.set("cwd", cwd);
        router.replace(`/agents/${encodeURIComponent(name)}?${params}`);
      }
    } catch {
  ...
  }, [agentName, content, scope, cwd, isNew, router]);
