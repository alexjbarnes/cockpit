...
- **`/context` in PTY sessions.** The command is now forwarded to the CLI (showing its live per-category breakdown and the actual window) instead of cockpit's readout of the picked size. It no longer leaves the session stuck on "processing", and ANSI colour codes are stripped from the output.
