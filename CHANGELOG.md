...
### Added
- **Plugin management.** A new Plugins page (under Settings) lists installed Claude Code plugins with their version and scope and lets you enable, disable, or uninstall each one. Actions run the CLI's own `claude plugin` commands, so install/git logic and the security blocklist stay owned by the CLI rather than reimplemented in cockpit.
